...
def bin_entries(data: bytes) -> list[bytes]:
    """Mirror PKHeX BinLinkerAccessor16's count + sequential u16 offsets."""
    if len(data) < 6:
        raise SystemExit("truncated BinLinkerAccessor16 resource")
    count = struct.unpack_from("<H", data, 2)[0]
    table_end = 4 + ((count + 1) * 2)
    if table_end > len(data):
        raise SystemExit(f"invalid BinLinkerAccessor16 count {count} for {len(data)} bytes")
    offsets = [struct.unpack_from("<H", data, 4 + i * 2)[0] for i in range(count + 1)]
    if offsets != sorted(offsets) or offsets[-1] > len(data):
        raise SystemExit("invalid BinLinkerAccessor16 offset table")
...
def read_levelup(data: bytes) -> list[list[int]]:
    ...
    for entry in bin_entries(data):
        if len(entry) % 3:
            raise SystemExit(f"invalid Gen II level-up entry size {len(entry)}")
        count = len(entry) // 3
...
def read_move_source(data: bytes) -> list[list[int]]:
    ...
    for entry in bin_entries(data):
        if len(entry) % 2:
            raise SystemExit(f"invalid Gen II move-source entry size {len(entry)}")
        count = len(entry) // 2
        out.append(list(struct.unpack_from(f"<{count}H", entry, 0)) if count else [])
    return out
...
def reverse_evolutions(data: bytes) -> dict[int, int]:
    ...
    for source in range(1, min(MAX_SPECIES + 1, len(entries))):
        ...
        if len(entry) % 8:
            raise SystemExit(f"invalid Gen II evolution entry size {len(entry)}")
        for off in range(0, len(entry), 8):
...
def direct_pools(crystal: bool) -> list[set[int]]:
    ...
    print(f"{suffix}: levelup entries={len(levelup)}, egg entries={len(egg)}")

    # PKHeX's level-up resource is indexed through all species. Egg resources are
    # deliberately sparse at the tail; LearnSource2GS/C treats species >= the
    # resource length as having no egg moves. Mirror that behavior by padding
    # missing trailing entries with empty pools rather than rejecting valid data.
    if len(levelup) != MAX_SPECIES + 1:
        raise SystemExit(f"{suffix}: expected 252 level-up entries, got {len(levelup)}")
    if len(egg) > MAX_SPECIES + 1:
        raise SystemExit(f"{suffix}: egg move resource unexpectedly has {len(egg)} entries")
    egg.extend([] for _ in range((MAX_SPECIES + 1) - len(egg)))
...
def main() -> None:
    ...
    if not rev:
        raise SystemExit("Gen II evolution resource produced no lineage links")
    print(f"Gen II evolution links={len(rev)}")
