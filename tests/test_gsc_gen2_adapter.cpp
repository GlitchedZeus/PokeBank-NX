...
namespace {
struct L { RegionLayout r; VersionFamily f; size_t size,trainer,money,boxIndex,boxNames,party,checksumEnd,checksum,gender; int boxes,cap,str; size_t spacing; int first; };
constexpr L IGS{RegionLayout::International,VersionFamily::GoldSilver,0x8000,0x2009,0x23DB,0x2724,0x2727,0x288A,0x2D68,0x2D69,0,14,20,11,0x450,7};
constexpr L IC {RegionLayout::International,VersionFamily::Crystal,0x8000,0x2009,0x23DC,0x2700,0x2703,0x2865,0x2B82,0x2D0D,0x3E3D,14,20,11,0x450,7};
constexpr L JGS{RegionLayout::Japanese,VersionFamily::GoldSilver,0x8000,0x2009,0x23BC,0x2705,0x2708,0x283E,0x2C8B,0x2D0D,0,9,30,6,0x54A,6};
constexpr L JC {RegionLayout::Japanese,VersionFamily::Crystal,0x10000,0x2009,0x23BE,0x26E2,0x26E5,0x281A,0x2AE2,0x2D0D,0x8000,9,30,6,0x54A,6};
...
void finalize(std::vector<uint8_t>& b,const L& l){uint16_t sum=0;for(size_t i=0x2009;i<=l.checksumEnd;i++)sum=uint16_t(sum+b[i]);b[l.checksum]=sum&0xFF;b[l.checksum+1]=sum>>8;}
...
void check(const L& l,SourceGame game,const char* id){auto raw=fixture(l);auto before=raw;auto p=parse(raw,game);assert(p);assert(p.save->metadata().sourceGameId==id);assert(p.save->metadata().region==l.r);assert(p.save->metadata().family==l.f);assert(p.save->metadata().boxCount==l.boxes);assert(p.save->metadata().boxCapacity==l.cap);assert(p.save->metadata().currentBox==2);assert(p.save->trainer().trainerId==12345);assert(p.save->trainer().money==54321);assert(p.save->party().size()==1);auto&m=p.save->party()[0];assert(m.species==25&&m.heldItem==173&&m.trainerId==12345&&m.level==5);assert(m.dvs[1]==2&&m.dvs[2]==10&&m.dvs[3]==10&&m.dvs[4]==10&&m.shiny);assert(m.friendship==70);assert(p.save->boxes().size()==size_t(l.boxes));assert(raw==before);}
}
int main(){
 check(IGS,SourceGame::Gold,"gold_gbc");check(IGS,SourceGame::Silver,"silver_gbc");check(IC,SourceGame::Crystal,"crystal_gbc");check(JGS,SourceGame::Gold,"gold_gbc");check(JC,SourceGame::Crystal,"crystal_gbc");
