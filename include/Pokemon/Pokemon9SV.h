/**
 * Pokemon9SV.h - Generation 9 Pokemon Data Class
 *
 * This file defines the Pokemon9SV class for Generation 9 Pokemon games:
 * - Pokemon Scarlet/Violet
 * - Pokemon Scarlet/Violet
 *
 * Pokemon9SV Format Details:
...
#ifndef Pokemon_Pokemon9_SV_H
#define Pokemon_Pokemon9_SV_H
...
namespace Pokemon {
    ...
    class Pokemon9SV final : public Pokemon
    {
    public:
        /**
         * Constructs a Pokemon9SV object from encrypted Pokemon data.
         *
         * Process:
         * 1. Decrypts the data using Gen9_SV decryption algorithm
         * 2. Stores decrypted data in internal buffer
         * 3. Creates span view for easy access
         *
         * @param raw Encrypted Pokemon data (SIZE_PARTY9_SV or SIZE_STORED9_SV bytes)
         */
         ...
         */
        ~Pokemon9SV() override = default;

        // Prevent copying (Pokemon data should not be accidentally copied)
        Pokemon9SV(const Pokemon9SV&) = delete;
        Pokemon9SV& operator=(const Pokemon9SV&) = delete;

        // Allow moving for efficient transfers
        Pokemon9SV(Pokemon9SV&&) noexcept = default;
        Pokemon9SV& operator=(Pokemon9SV&&) noexcept = default;
        ...
        std::unique_ptr<Pokemon> clone() const override {
            ...
            std::byte* enc = encryptArray9SV(std::span<const std::byte>(data.data(), dataSize), ec);
            auto c = std::make_unique<Pokemon9SV>(std::span<const std::byte>(enc, dataSize));
            delete[] enc;
            return c;
        }

        /** Storage-format game group (this subclass), NOT the origin Version byte. */
        Enums::GameVersion getGameGroup() const noexcept override { return Enums::GameVersion::SV; }
        ...
        uint16_t calculateChecksum() const noexcept override {
            ...
            const size_t checksumEnd = std::min(dataSize, SIZE_STORED9_SV);
