...
namespace Pokemon {
    ...
    /**
     * Pokemon8SWSH - Generation 8 Pokemon Class
     *
    ...
    public:
        ...
        std::unique_ptr<Pokemon> clone() const override {
            ...
            std::byte* enc = encryptArray8SWSH(std::span<const std::byte>(data.data(), dataSize), ec);
            auto c = std::make_unique<Pokemon8SWSH>(std::span<const std::byte>(enc, dataSize));
            delete[] enc;
            return c;
        }

        /** Storage-format game group (this subclass), NOT the origin Version byte. */
        Enums::GameVersion getGameGroup() const noexcept override { return Enums::GameVersion::SWSH; }
         ...
         * Location: 0x72 + slot*2 (2 bytes each, slots 0-3).
        ...
        /** Game of origin (Version byte). Location: 0xDE. */
        uint8_t originGame() const noexcept override { return static_cast<uint8_t>(data[0xDE]); }
        void setOriginGame(uint8_t version) noexcept override { data[0xDE] = static_cast<std::byte>(version); refreshChecksum(); }
        ...
        /** Language id. Location: 0xE2. */
        uint8_t language() const noexcept override { return static_cast<uint8_t>(data[0xE2]); }
        void setLanguage(uint8_t value) noexcept override { data[0xE2] = static_cast<std::byte>(value); refreshChecksum(); }
        ...
        {
            (void)setNicknameChecked(value); // invalid input is rejected; never silently truncated
        }
