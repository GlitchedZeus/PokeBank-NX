/**
 * Gen8Encryption.h - Generation 9 Pokemon Encryption/Decryption Utilities
 *
 ...
 *
 * Pokemon data in Gen 9 is stored in an encrypted format using:
...
#ifndef ENCRYPTION_GEN9_ENCRYPTION_H
#define ENCRYPTION_GEN9_ENCRYPTION_H
 ...
 */
constexpr size_t BLOCK_9COUNT = 4;
 ...
 */
constexpr size_t SIZE_9BLOCK = 0x50;

/**
 * Size of stored Pokemon data (Gen 9).
 * Includes 8-byte header + 4 blocks = 328 bytes (0x148).
 */
constexpr size_t SIZE_9STORED = 8 + (BLOCK_9COUNT * SIZE_9BLOCK);
 ...
 */
void shuffleArray9(std::span<const std::byte> data, std::span<std::byte> result, uint32_t sv);
 ...
 *
 * @param encryptedData The encrypted Pokemon data (SIZE_9PARTY bytes)
 * @return Pointer to decrypted data (caller must delete[])
 */
std::byte* decryptArray9(std::span<const std::byte> encryptedData);
