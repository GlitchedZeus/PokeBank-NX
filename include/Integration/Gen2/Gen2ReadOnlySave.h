...
struct PokemonRecord {
    ...
    std::array<uint8_t, 5> dvs{}; // HP / Atk / Def / Spe / Special
    ...
    bool isEgg = false;
    bool shiny = false;
    uint8_t gender = 2; // 0 male / 1 female / 2 unknown-or-genderless; resolved by presentation personal data.
    uint8_t form = 0;   // Unown A-Z = 0..25, else 0.
    std::array<uint8_t, 48> rawBody{};
