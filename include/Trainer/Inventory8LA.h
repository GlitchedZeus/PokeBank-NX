...
namespace Trainer {
    ...
    inline PouchInfo8LA getPouchInfo8LA(PouchType8LA pouch) {
        switch (pouch) {
            case PouchType8LA::Regular:  return {"Everyday Items"};
            case PouchType8LA::KeyItems: return {"Key Items"};
            case PouchType8LA::Stored:   return {"Stored Items"};   // base storage-box overflow
            case PouchType8LA::Recipes:  return {"Recipes"};        // crafting recipes
        }
