...
TrainerViewScreen::TrainerViewScreen(Trainer& trainer, const std::string& titleName, const std::string& backupDir, u64 titleId)
...
void TrainerViewScreen::update(const PadState& pad) {
    ...
    if (saveConfirmActive) {
        if (kDown & HidNpadButton_A) {
            ...
            bool saveSuccess = saveTrainerInfo(trainer, backupDir.c_str(), titleId);
            ...
            if (kDown & HidNpadButton_A) {
                ...
                if (pokemon) {
                    ...
                    if (pokemonDetailsSelectedField == 3) {
                        ...
                        logInfoToFile("Current shiny status: " + std::to_string(currentShiny));
        ...
        if (selectedMode == ViewMode::Party) {
            ...
            if (kDown & HidNpadButton_A) {
                ...
                if (selectedPartyIndex >= 0 && selectedPartyIndex < static_cast<int>(trainer.party.size())) {
                    const PKM* pokemon = trainer.party[selectedPartyIndex].get();
