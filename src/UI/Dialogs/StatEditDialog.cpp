...
namespace Dialogs {
    void drawStatEditDialog(TrainerViewScreen& screen, PKSEFramebuffer& fb) {
        ...
        int cy = drawDialogFrame(fb, x, y, w, h, std::string("Edit ") + statName, Colors::Text);
        ...
        if (usesAV) {
            fb.drawText(labelX, cy, "Awakening Values (each 0-200, no total cap)", Colors::TextDim, TextStyle::Caption);
        } else {
            uint8_t currentEV = 0;
            ...
            fb.drawText(labelX, cy, "Total EVs: " + std::to_string(projectedTotal) + " / 510",
        ...
        drawEditChoiceButton(screen, fb, x + w - 24 - cbw, cby, cbw, cbh, "A", "Save", 1);
    }
}
}
