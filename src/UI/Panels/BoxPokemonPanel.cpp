...
namespace Panels {
    ...
    void drawBoxPokemon(TrainerViewScreen& screen, PKSEFramebuffer& fb, int x, int y, int width, int height) {
        ...
        if (screen.selectedBoxIndex < 0 || screen.selectedBoxIndex >= static_cast<int>(screen.trainer.boxes.size())) {
            ...
            fb.drawText(x + 20, y + 70, "No box data available", Colors::TextDim);
            return;
        }
    ...
    void drawBoxSummaryPanel(TrainerViewScreen& screen, PKSEFramebuffer& fb, int x, int y, int width, int height) {
        ...
        { int tw, th; fb.measureText("Summary", tw, th, TextStyle::Body);
          fb.drawText(x + (width - tw) / 2, y + (headerH - th) / 2, "Summary", Colors::Text); }
        ...
        fb.drawRoundedRect(x, y, width, height, 16, Colors::Border, 1);
        if (!p) {
            int tw, th; fb.measureText("No Pokémon selected", tw, th, TextStyle::Caption);
            fb.drawText(x + (width - tw) / 2, y + height / 2, "No Pokémon selected", Colors::TextDim, TextStyle::Caption);
            return;
        }
        ...
        // Lv + zero-padded dex no.
        {
            ...
            std::string sub = "Lv. " + std::to_string(p->level()) + "    No. " + dex;
