...
namespace Panels {
    ...
    const char* pouchDisplayName(GameVersion gameGroup, int category) {
        ...
        else if (gameGroup == GameVersion::GSC)  name = PokeVault::Integration::Gen2::inventoryCategoryName(static_cast<size_t>(category));
    ...
    void drawItems(TrainerViewScreen& screen, PKSEFramebuffer& fb, int x, int y, int width, int height) {
        ...
        const char* pouchName = rbySource
            ...
            : gscSource ? PokeVault::Integration::Gen2::inventoryCategoryName(static_cast<size_t>(screen.selectedCategory))
        ...
        for (int i = startIdx; i < endIdx; ++i) {
            ...
            const char* itemName = (gameGroup == GameVersion::RBY)
                ? Names::getItemNameG1(item.itemId)
                : (gameGroup == GameVersion::GSC)
                    ? Names::getItemNameG2(item.itemId)
                    : (gameGroup == GameVersion::FRLG)
                        ? Names::getItemNameG3(item.itemId)
                        : getItemName(item.itemId);
            fb.drawText(nx, ry + (tileH - fb.lineHeight(TextStyle::Body)) / 2, itemName, nameCol, TextStyle::Body);
            if (gameGroup != GameVersion::RBY && gameGroup != GameVersion::GSC) if (uint16_t tmMove = Names::getTMMove(gameGroup, item.itemId)) {
