...
namespace Panels {
    ...
    void drawPartyPokemon(PKSEFramebuffer& fb, const Trainer::Trainer& trainer,
        ...
        fb.drawText(x + 16, y + 12, "Party", Colors::Text, TextStyle::Heading);
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (!pokemon || pokemon->speciesID() == 0) {
                fb.drawText(cardX + pad, cardY + slotH / 2 - 12,
                            "Slot " + std::to_string(i + 1) + ":  Empty", Colors::TextDim);
                continue;
            }
            ...
            if (gender != "?" && gender != "Genderless" && gender != "") {
            ...
            std::string lvl = "Lv. " + std::to_string(pokemon->level());
            ...
            const StatRow rows[6] = {
                ...
                {"Atk", pokemon->baseATK(), pokemon->ivATK(), av ? pokemon->avATK() : pokemon->evATK(), pokemon->statATK()},
                {"Def", pokemon->baseDEF(), pokemon->ivDEF(), av ? pokemon->avDEF() : pokemon->evDEF(), pokemon->statDEF()},
                {"SpA", pokemon->baseSPA(), pokemon->ivSPA(), av ? pokemon->avSPA() : pokemon->evSPA(), pokemon->statSPA()},
                {"SpD", pokemon->baseSPD(), pokemon->ivSPD(), av ? pokemon->avSPD() : pokemon->evSPD(), pokemon->statSPD()},
                {"Spe", pokemon->baseSPE(), pokemon->ivSPE(), av ? pokemon->avSPE() : pokemon->evSPE(), pokemon->statSPE()},
            };
            ...
            fb.drawText(cBase, tY, "Base",           colHdr, TextStyle::Caption);
            fb.drawText(cIV,   tY, "IV",             colHdr, TextStyle::Caption);
            fb.drawText(cEV,   tY, av ? "AV" : "EV", colHdr, TextStyle::Caption);
            fb.drawText(cStat, tY, "Stat",           colHdr, TextStyle::Caption);
            tY += lineH;
