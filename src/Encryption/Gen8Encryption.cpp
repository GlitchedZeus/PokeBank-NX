/**
 * Gen8Encryption.cpp - Generation 8 Pokemon Encryption/Decryption Implementation
 *
 * Implementation of encryption and decryption utilities for Generation 8 Pokemon data.
 * See Gen8Encryption.h for detailed documentation.
 */
...
void shuffleArray8(std::span<const std::byte> data, std::span<std::byte> result, uint32_t sv)
{
     ...
     *
     * The shuffle value (SV) determines how the blocks were scrambled.
    ...
    const uint32_t index = sv * BLOCK_8COUNT;
    ...
    const auto end = start + (SIZE_8BLOCK * BLOCK_8COUNT);
    ...
    for (uint32_t block = 0; block < BLOCK_8COUNT; block++)
    {
        // blockPosition[index + block] tells us which block is in position 'block'
        const int srcBlockIndex = blockPosition[index + block];
        const size_t srcOffset = start + (SIZE_8BLOCK * srcBlockIndex);
        const size_t destOffset = start + (SIZE_8BLOCK * block);

        // Copy one block from shuffled position to correct position
        std::memcpy(result.data() + destOffset, data.data() + srcOffset, SIZE_8BLOCK);
    }
}
...
std::byte* decryptArray8(std::span<const std::byte> encryptedData)
{
    ...
    cryptPKM(mutableSpan, pv, SIZE_8BLOCK, BLOCK_8COUNT);
    ...
    shuffleArray8(mutableSpan, resultSpan, sv);
...
std::byte* encryptArray8(std::span<const std::byte> decryptedData, uint32_t pv)
{
    ...
    for (uint32_t block = 0; block < BLOCK_8COUNT; block++)
    {
        ...
        const size_t srcOffset = start + (SIZE_8BLOCK * block);
        const size_t destOffset = start + (SIZE_8BLOCK * destBlockIndex);

        // Copy one block from decrypted position to shuffled position
        std::memcpy(shuffledSpan.data() + destOffset, decryptedData.data() + srcOffset, SIZE_8BLOCK);
    }

    // Step 2: Apply XOR cipher (symmetric operation)
    cryptPKM(shuffledSpan, pv, SIZE_8BLOCK, BLOCK_8COUNT);
