...
std::byte* decryptArray8SWSH(std::span<const std::byte> encryptedData)
{
    ...
    cryptPokemon(mutableSpan, pv, SIZE_BLOCK8_SWSH, BLOCK_COUNT8_SWSH);
    ...
    shuffleArray8SWSH(mutableSpan, resultSpan, sv);
...
std::byte* encryptArray8SWSH(std::span<const std::byte> decryptedData, uint32_t pv)
{
    /**
     * Main encryption function for Generation 8 Pokemon data.
     *
     * This is the reverse of decryptArray8SWSH. It takes unencrypted, unshuffled
    ...
    const uint32_t index = sv * BLOCK_COUNT8_SWSH;
    constexpr uint32_t start = 8;
    ...
    cryptPokemon(shuffledSpan, pv, SIZE_BLOCK8_SWSH, BLOCK_COUNT8_SWSH);
