...
std::vector<FRLGSourceCard> buildGSCSourceCards(const GSCDiscoveryResult& discovery) {
    ...
    for (size_t index = 0; index < discovery.sources.size(); ++index) {
        ...
        if (!game || game->platform != Games::Platform::GameBoyColor ||
        ...
        if (cardIt == cards.end()) {
            cards.push_back({
                std::string(game->id), std::string(game->title),
                std::string(Games::platformName(game->platform)), "RETROARCH",
                std::string(game->id), {},
            });
            instanceKeys.emplace_back();
            cardIndex = cards.size() - 1;
        } else cardIndex = static_cast<size_t>(std::distance(cards.begin(), cardIt));
        ...
        const std::string fingerprint = source.contentFingerprint.empty() ? std::string("unavailable") : source.contentFingerprint;
        const std::string details = (strictTrainer.name.empty() ? std::string("Unknown trainer") : strictTrainer.name) +
...
std::vector<FRLGSourceCard> buildGSCSourceCardsForProfile(
    ...
    for (auto& card : cards) {
        card.instances.erase(std::remove_if(card.instances.begin(), card.instances.end(),
            [&](const auto& instance) { return !bindings.isVisibleTo(instance.sourceIdentity, profileIdentity); }),
            card.instances.end());
    ...
    cards.erase(std::remove_if(cards.begin(), cards.end(), [](const auto& card) { return card.instances.empty(); }), cards.end());
    return cards;
}
...
const GSCSource* resolveGSCSaveInstance(
    ...
    if (instance.kind != LegacySaveInstanceKind::BatterySave || instance.sourceIndex >= discovery.sources.size()) return nullptr;
    const auto& source = discovery.sources[instance.sourceIndex];
    if (!source.ready() || source.gameId != card.gameId) return nullptr;
    if (instance.normalizedPath != source.normalizedPath || instance.sourceIdentity != stableIdentity(source) ||
        instance.fileSize != source.fileSize || instance.modifiedTime != source.modifiedTime ||
    ...
    if (!game || game->platform != Games::Platform::GameBoyColor ||
        game->support != Games::SourceSupport::ReadOnly || game->switchTitleId != 0) return nullptr;
    return &source;
}
